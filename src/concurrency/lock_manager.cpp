...
auto LockManager::LockTable(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool {
  ...
  for (auto request : lock_request_queue->request_queue_) {  // NOLINT
    ...
    // A transaction can hold no or at most one lock on a table.
    // If the transaction already holds a lock on the table, we might update it.
    // If the transaction does not hold a lock on the table, we will add a new lock request.
    if (request->txn_id_ == txn->GetTransactionId()) {
      ...
      // If an exclusive lock is granted, no other lock requests in the queue can be granted.
      // If the lock mode is not exclusive, we notify all waiting transactions.
  ...
  // The transaction does not hold any lock on the table. We need to add a new lock request.
...
auto LockManager::LockRow(Transaction *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool {
  // These lock modes are n ot allowed on rows.
  ...
  for (auto request : lock_request_queue->request_queue_) {  // NOLINT
    if (request->txn_id_ == txn->GetTransactionId()) {
      if (request->lock_mode_ == lock_mode) {
        // This transaction already requests the same lock on the row.
        lock_request_queue->latch_.unlock();
        return true;
      }

      // This transaction is trying to upgrade a lock.
      if (lock_request_queue->upgrading_ != INVALID_TXN_ID) {
        lock_request_queue->latch_.unlock();
        txn->SetState(TransactionState::ABORTED);
        throw TransactionAbortException(txn->GetTransactionId(), AbortReason::UPGRADE_CONFLICT);
